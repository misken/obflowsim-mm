...
def process_command_line():
    ...
    # Create the parser
    parser = argparse.ArgumentParser(prog='mm_performance_curves',
                                     description='Generate predictions from fitted models')
    ...
    parser.add_argument(
        "pkl_path", type=str,
        help="Path containing pkl files created from metamodel fitting"
    )

    parser.add_argument(
        "X_data_path", type=str,
        help="Path to directory in which to write X data for predictions"
    )

    parser.add_argument(
        "output_path", type=str,
        help="Path to write output csv files"
    )
...
def main(argv=None):
    ...
    mm_exp = mm_args.mm_experiment
    pred_exp = mm_args.predict_experiment
    scenario_input_path_filename = mm_args.scenario_input_path_filename
    pkl_path = mm_args.pkl_path
    X_data_path = mm_args.X_data_path
    output_path = mm_args.output_path

    obs_pkl_path_filename = Path(f'{pkl_path}/{mm_exp}_obs_results.pkl')
    ldr_pkl_path_filename = Path(f'{pkl_path}/{mm_exp}_ldr_results.pkl')
    pp_pkl_path_filename = Path(f'{pkl_path}/{mm_exp}_pp_results.pkl')

    # Path to output csv file
    performance_curve_predictions_path_filename = \
        Path(f'{output_path}/pc_predictions_{pred_exp}.csv')

    performance_curve_predictions_long_path_filename = \
        Path(f'{output_path}/pc_predictions_{pred_exp}_long.csv')
    ...
    scenarios_x_dfs = make_x_scenarios(input_scenarios_df, mm_exp, X_data_path)
            ...
            for measure in ['occmean', 'occp95', 'probblocked', 'condmeantimeblocked']:
                ...
                if f'{unit}_{measure}_{f}_{m}_results' in unit_fit_results:
                    ...
                    scenarios_predictions_df[f'pred_{unit}_{measure}_{f}_{m}'] = \
                        unit_fit_results[f'{unit}_{measure}_{f}_{m}_results']['model'].predict(
                            scenarios_x_dfs[f'X_{unit}_{fullfeature}'])
